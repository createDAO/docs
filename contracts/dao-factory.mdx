...
The factory contains implementation contracts for **DAOToken**, **DAOGovernor**, and **DAOTimelock**, deployed once. When creating a new DAO, it clones these implementations (EIP-1167) and initializes them so the three contracts work together as a complete governance system.
...
flowchart TD
    ...
    subgraph CreateDAO["createDAO()"]
        ...
        CloneTimelock[Clone DAOTimelock → 45 bytes proxy]
        ...
        CloneToken --> CloneTimelock
        CloneTimelock --> CloneGovernor
...
- `timelock`: Address of the DAOTimelock proxy (treasury)
...
2. **Clone DAOTimelock** (EIP-1167)
   - Creates minimal proxy pointing to `timelockImplementation`
   - Initializes the timelock delay and admin
...
:::note
The current factory version also exposes `timelockImplementation()`.
:::
...
| Treasury (DAOTimelock) | 99% | Secure storage - distributed through governance proposals |
...
const factory = await ethers.getContractAt(
    "DAOFactory",
    "0x742236f55a5e43B1AAec404527cE6986E02cD700" // Ethereum mainnet
);
